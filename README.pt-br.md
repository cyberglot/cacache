...
[`cacache`](https://github.com/zkat/cacache) é uma biblioteca de Node.js para
gerenciar caches locais em disco, com acesso tanto com chaves únicas como
hashes. É super rápida, excelente com acesso concorrente, e jamais te dará dados incorretos, ainda que os arquivos de cache sejam corrompidos ou
manipulados diretamente.

O propósito original de cacache era ser usado como cache local da
[npm](https://npm.im/npm), mas pode ser facilmente usado por conta própria.

_Traduções: [English](README.md), [Español](README.es.md)_
...
* [API](#api)
  * [Usando a API em português](#localized-api)
  * Leitura
    * [`ls`](#ls)
    * [`ls.stream`](#ls-stream)
    * [`get`](#get-data)
  ...
  * Integridade
    * [Integridade de subfonte](#integrity)
    * [`verify`](#verify)
...
const tarball = '/caminho/para/minha-tar.tgz'
const cachePath = '/tmp/minha-cache-de-brinquedo'
const key = 'minha-chave-única-1234'

// Cacheia ! Usa `cachePath` como raiz da cache.
cacache.put(cachePath, chave, '10293801983029384').then(integrity => {
  console.log(`Conteúdo salvo em ${cachePath}.`)
})

const destination = '/tmp/minhatar.tgz'

// Copia o conteúdo da cache para outro arquivo, mas dessa vez com streams.
cacache.get.stream(
  cachePath, chave
).pipe(
...
// A mesma coisa, mas acessando o conteúdo diretamente, sem tocar no índice.
cacache.get.byDigest(cachePath, integrity).then(data => {
  fs.writeFile(destination, data, err => {
    console.log('Dados do tarball obtidos com base no seu sha512, e escrito em outro arquivo!')
  })
})
```
